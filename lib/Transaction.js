...
function Transaction(data) {
  ...
  if (data.clam_speech && data.clam_speech.length <= 528 && data.version == 2) {
    this.clam_speech = data.clam_speech;
  }

  if (data.time && data.version == 2) {
    this.time = data.time;
  }
...
Transaction.prototype.serialize = function serialize() {
  ...
  if (this.clam_speech && this.clam_speech.length > 0 && this.version == 2) {
    bufs.push(util.varIntBuf(this.clam_speech.length));
    bufs.push(this.clam_speech);
  }
...
Transaction.prototype.parse = function(parser) {
  ...
  if (this.version == 2) { // clam speech transaction
    sLen = parser.varInt(); // clam speech len
    if (sLen <= 528 && sLen > 0) {
      this.clam_speech = parser.buffer(sLen);
    }
  }
